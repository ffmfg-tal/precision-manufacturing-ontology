...
| cad_model | [schemas/cad_model.yaml](schemas/cad_model.yaml) | [provenance-and-verification](extensions/provenance-and-verification.md) | [matrix row](reference/entity-matrix.md#cad_model) |
...
| consumable_inventory | [schemas/consumable_inventory.yaml](schemas/consumable_inventory.yaml) | [consumable-inventory](extensions/consumable-inventory.md) | [matrix row](reference/entity-matrix.md#consumable_inventory) |
...
| derived_view | [schemas/derived_view.yaml](schemas/derived_view.yaml) | [provenance-and-verification](extensions/provenance-and-verification.md) | [matrix row](reference/entity-matrix.md#derived_view) |
...
| manufacturing_feature | [schemas/manufacturing_feature.yaml](schemas/manufacturing_feature.yaml) | [provenance-and-verification](extensions/provenance-and-verification.md) | [matrix row](reference/entity-matrix.md#manufacturing_feature) |
...
| provenance | *(value-object / mixin — [schemas/provenance.yaml](schemas/provenance.yaml))* | [provenance-and-verification](extensions/provenance-and-verification.md) | [matrix row](reference/entity-matrix.md#provenance) |
...
| tracked_document | [schemas/tracked_document.yaml](schemas/tracked_document.yaml) | [as9100-qms-layer](extensions/as9100-qms-layer.md) | [matrix row](reference/entity-matrix.md#tracked_document) |
...
| verification_event | [schemas/verification_event.yaml](schemas/verification_event.yaml) | [provenance-and-verification](extensions/provenance-and-verification.md) | [matrix row](reference/entity-matrix.md#verification_event) |
...
- **Entity count:** 133. The 2026-05-31 provenance/design-ingestion pass added 4 entities (`cad_model`, `derived_view`, `manufacturing_feature`, `verification_event`) to reach 132; the AS9100D §7.5 controlled-document register `tracked_document` (schema authored 2026-04-28) was integrated 2026-06-04 to reach 133. The `provenance` value-object (a cross-cutting mixin embedded on asserted entities, not a top-level identified entity) is indexed but not counted. See [provenance-and-verification](extensions/provenance-and-verification.md), [as9100-qms-layer](extensions/as9100-qms-layer.md), and Decision 1.10.
- **Schemas:** 121 entities have machine-readable schemas (120 before the 2026-06-04 `tracked_document` integration; the authoritative count is the `has_schema` field in `reference/entity-inventory.yaml`, against which the prior in-doc figure of 117 was stale). Consumable-inventory addition (2026-05-16): `consumable_inventory` (in new `schemas/consumable_inventory.yaml`) covers unit-tracked non-serialized tooling and shop-floor consumables — cutters, inserts, grinding wheels, sockets, bits, PPE, cleaning supplies — with a `consumption_mode` discriminator (ConsumedDiscarded / ConsumedRecoverable / IssuedReturnable) that disambiguates lifecycle. Distinct from `material` (production raw stock with heat/lot/DFARS traceability). Fills the cutter-stock-tracking gap previously noted on `cutter_definition`. Controlled-document addition (2026-04-28): `tracked_document` (in `schemas/tracked_document.yaml`) covers AS9100D §7.5 control of documented information for QMS procedures, work instructions, customer quality manuals, T&Cs, supplier quality requirements, training material, and external references. Distinct from `certification_document` (record of external evidence against a material lot or process run). AS9100D QMS additions (9): `audit_finding`, `audit_plan`, `corrective_action`, `lessons_learned`, `operational_risk_assessment` (in `schemas/corrective_action.yaml`); `contract_review_event`, `customer_complaint`, `requirement_change_notification` (in `schemas/orders.yaml`); `product_release_authorization` (added to `schemas/inspection.yaml`). Field additions to existing schemas: `schemas/customer_purchase_order.yaml` (`special_requirements[]`, `operational_risk_flags[]`, `contract_review_id`); `schemas/quality.yaml` (`nonconformance_report`: `counterfeit_flag`, `scar_supplier_id`, `corrective_action_id`; `first_article_inspection`: `repeat_reason_ecn_id`); `schemas/inspection.yaml` (`inspection_event`: `authorized_by`, `release_type`); `schemas/approved_supplier_list_entry.yaml` (`formal_approval_status`, `approval_scope_description`, `last_performance_review_date`, `performance_review_cadence_months`, `counterfeit_risk_tier`).
