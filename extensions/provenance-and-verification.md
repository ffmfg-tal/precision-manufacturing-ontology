...
The crucial property: **what is permanent is the accountability seam, not the verification *burden*.** It is tempting to read human review as a crutch to be removed once the AI is good enough — and the burden of reviewing *everything* genuinely does fall away as track records accumulate (the `Next` and `End state` above); where a machine becomes demonstrably safer than a person, you are obligated to lean on it, autopilot rather than pilot. What does *not* fall away is the signed seam where a named human takes ownership of the outcome. In a regulated shop that seam is structural — it answers to a regulator and a court, not to model capability — which is exactly what makes the inversion *safe and certifiable*, and why it is the last construct to move. The human's position in the loop changes; the accountable human does not leave it. `verification_event` is the technical embodiment of that seam, which is why it is first-class, immutable, and signed.
