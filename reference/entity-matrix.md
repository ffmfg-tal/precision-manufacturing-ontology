...
Sources: `reference/entity-inventory.yaml` (133 entities), `reference/composition-archetypes.md` (required fields per archetype), all files in `extensions/`, all files in `schemas/`, `standards/*.md`, `reference/domain-map.md`, `reference/carbon-os-mapping.md`.

> **Note (2026-06-04):** This matrix's prose narrative and the per-column tally at the bottom predate the consumable-inventory (2026-05-16), provenance/design-ingestion (2026-05-31), and tracked_document (2026-06-04) additions. The authoritative current counts are **133 entities, 121 with standalone schemas** (the `has_schema` field in `reference/entity-inventory.yaml`). The per-entity dimension blocks below are current for the entities they cover; a full per-column recount across all 133 entities is the one deliberately deferred item — see `reference/residual-gaps.md`.
...
The ontology's strength is narrative and compositional: every archetype walks end-to-end, every extension carries a coherent entity model in prose, and the four-layer architecture cleanly separates standards from manufacturer-layer gap fills. As of mid-2026 the mechanical side has largely caught up: 121 of 133 entities carry machine-readable schemas (the remaining 12 are intentional — external standards, value types, aliases, embedded sub-objects, and one removed-from-canon entity), lifecycle entities have state graphs across the assembly, inspection, quality, and orders domains, and the major taxonomic questions have been resolved in `reference/taxonomic-decisions.md`. The remaining mechanical debt is concentrated and tracked: a full per-column tally recount, the UUID-table cross-reference pass, and a handful of state-machine graphs (see the weakest-columns note below and `reference/residual-gaps.md`). The historical phase-update notes that follow record how the spec reached this state.
...
1. **Schema authoring** — effectively complete (121 of 133 entities have schemas as of 2026-06-04). The 12 null-schema entries are intentional: external standards (machine_event, qif_results_document, x12_856_asn), ITAR/value references (usml_category, country, heat_number, spec_body, uns_number), aliases (fai_package, process_identification), an embedded sub-object (fai_form_1_header), and removed-from-canon (item). A future deepening pass may promote fai_form_1_header and fai_package to standalone schemas.
...
**Stale — pending recount.** The numbers in this table were last computed 2026-04-24 across 117 entities and do not reflect the consumable-inventory, provenance/design-ingestion, or tracked_document additions. The authoritative current totals are **133 entities, 121 with standalone schemas** (`reference/entity-inventory.yaml`, `has_schema` field). A full per-column recount across all 133 entities is a deliberately deferred item (`reference/residual-gaps.md`); the per-entity dimension blocks above are current for the entities they cover.
...
- **Schema defined** now at 106 ✅ / 11 🔴. Phase 3.4 added `schemas/assembly.yaml` (torque_specification, torque_sequence, torque_readings_record, witness_inspection, kit_record) and `schemas/inspection.yaml` (inspection_event, calibration_record, tool_gauge, key_characteristic). Phase 2 added schemas for all 29 new entities (Domains 9–12, Assembly Hardware, Operator Qualification). InspectAI added 6 new schemas (characteristic.yaml, measurement_result.yaml, quality_tools.yaml). Remaining 11 🔴: external standards (machine_event, qif_results_document, x12_856_asn), string/enum per Decision 1.7 (country, heat_number, spec_body, uns_number, usml_category), removed-from-canon (item), and no-schema aliases/embedded sub-objects with no independent schema (fai_form_1_header, fai_package, quality_engineer, supplier_approval, process_identification, export_compliance_declaration) — but these are ⬜-equivalent where the schema is provided by the parent entity or alias target. The 🔴 count above reflects entities with `has_schema: null` and no alias resolution.
