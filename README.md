...
Active. Phase 2 (12 domains, 117 entities) completed 2026-04-24; InspectAI-derived quality entities added same day. AS9100D QMS layer alignment pass completed 2026-04-26: 9 new entities added (`audit_finding`, `audit_plan`, `contract_review_event`, `corrective_action`, `customer_complaint`, `lessons_learned`, `operational_risk_assessment`, `product_release_authorization`, `requirement_change_notification`), bringing total to 126. Controlled-document register added 2026-04-28: `tracked_document` (total: 127). Shop-floor consumable register added 2026-05-16: `consumable_inventory` (total: 128) — unit-tracked non-serialized tooling and consumables (cutters, inserts, grinding wheels, PPE, sockets) with a `consumption_mode` discriminator distinguishing ConsumedDiscarded / ConsumedRecoverable / IssuedReturnable lifecycles; deliberately distinct from `material`. Schema coverage: 117 of 128 entities fully specified; remaining 11 are intentional (external standards not authored here, string/enum type decisions, aliases, removed-from-canon). New schemas: `schemas/orders.yaml`, `schemas/corrective_action.yaml`, `schemas/tracked_document.yaml`, `schemas/consumable_inventory.yaml`; field additions to `schemas/customer_purchase_order.yaml`, `schemas/quality.yaml`, `schemas/inspection.yaml`, `schemas/approved_supplier_list_entry.yaml`, `schemas/tool_room.yaml` (cutter_definition.tracked_in_inventory relationship). Prose: `extensions/as9100-qms-layer.md`, `extensions/consumable-inventory.md`. Machine-readable reference files updated 2026-04-24: `reference/entity-inventory.yaml` (117 entries) and `reference/relationship-graph.yaml` (355 edges) — pending full update to 128 entries and ~390 edges. See [`reference/residual-gaps.md`](reference/residual-gaps.md) for deferred items and architectural decisions.

**Provenance & verification substrate added 2026-05-31** (Decision 1.10): four new entities — `cad_model`, `derived_view`, `manufacturing_feature`, `verification_event` — plus the cross-cutting `provenance` value-object (a mixin, not counted), bringing the entity total to **132**. This is the epistemic substrate: every asserted fact (extracted, inferred, computed, measured) carries how it was established, with what confidence, from what source, and whether a human has verified it — the precondition for putting AI in the loop on regulated work. It also closes the design-ingestion gap at the front of the digital thread (model + drawing → structured features and requirements), demonstrated by `reference/composition-archetypes.md` **Walk 8 — Design Ingestion / CAM Programming**, and articulates the platform's **human-supported-AI** trajectory (AI runs the manufacturing-engineering loop; humans verify, judge, and own). New schemas: `schemas/provenance.yaml`, `schemas/cad_model.yaml`, `schemas/derived_view.yaml`, `schemas/manufacturing_feature.yaml`, `schemas/verification_event.yaml`. New prose: `extensions/provenance-and-verification.md`. Field additions to existing entities (`characteristic`, `part_specification`, `measurement_result`, `operation`) are specified in the extension doc + Decision 1.10 as **staged** — they land in their schemas in lockstep with implementation codegen to avoid drift. The machine-readable reference files gain the four new entities; the pre-existing 117→128 inventory backlog remains as noted above.
